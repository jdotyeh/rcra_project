# =============================================================================
# 08_ce_master_prototype.R
#
# Prototype master file for the Compliance, Monitoring & Enforcement module:
...
#
# The replication pipeline builds the equivalent CE_MASTER.csv in
# code/modules/02_modular_master_files/rcrainfo/02_ce_master.R; this
...
cme_dir  <- "data/rcrainfo/ce"
out_file <- "output/diagnostics/cme_master.csv"
dir.create(dirname(out_file), showWarnings = FALSE, recursive = TRUE)

read_cme <- function(file) {
  df <- read_csv(file.path(cme_dir, file),
                 col_types = cols(.default = "c"), show_col_types = FALSE)
  names(df) <- gsub(" ", "_", names(df))
  df
}

reporting <- read_cme("CE_REPORTING.csv")
citation  <- read_cme("CE_CITATION.csv") |>

...
#
# Each workbook can also be mirrored to a Google Sheet. The push is skipped
# unless the environment variable RCRA_PUSH_GSHEET is set to "true" (it needs
# a one-time interactive Drive login), so batch runs never block on OAuth.
#
...
# Goal: a Google Sheet (and any linked table in a Google Doc) that LOOKS like
# the .xlsx -- merges, fills, borders, fonts, column widths. Cell-value writes
# (the old approach) lose all of that, so instead we upload the .xlsx and let
# Drive CONVERT it into the existing Google Sheet: same Sheet ID is kept (Doc
# link never breaks) and Google's xlsx import preserves the formatting.
#
# Pushes only run when RCRA_PUSH_GSHEET=true (see gsheet_push_enabled below).
#
# Auth: needs the Drive scope (broader than spreadsheets-only). Run gsheet_auth()
# ONCE interactively (opens a browser); the token caches for later runs.
gsheet_push_enabled <- function()
  tolower(Sys.getenv("RCRA_PUSH_GSHEET")) %in% c("1", "true", "yes")

gsheet_auth <- function(email = "junliangyeh@gmail.com") {
...
# Convert-upload one workbook into an EXISTING Google Sheet (keeps its ID).
push_xlsx_to_gsheet <- function(xlsx_path, gsheet_id) {
  if (!requireNamespace("googledrive", quietly = TRUE))
    stop("googledrive not installed; run install.packages('googledrive')")
  if (!googledrive::drive_has_token()) gsheet_auth()
  googledrive::drive_update(googledrive::as_id(gsheet_id), media = xlsx_path)
  cat(sprintf("Converted %s -> Google Sheet %s\n", basename(xlsx_path), gsheet_id))
}

# Combine several formatted workbooks into one .xlsx and convert-upload it to a
# single Google Sheet. `specs` = list(list(path=, prefix=)); each source tab
# becomes paste0(prefix, tab). Used when >1 module shares one Sheet (WIETS).
push_combined_to_gsheet <- function(specs, gsheet_id) {
  out <- wb_workbook()
  for (sp in specs) {
    src <- wb_load(sp$path)
    for (s in src$get_sheet_names())
      out <- wb_clone_worksheet(out, old = s, new = paste0(sp$prefix, s), from = src)
  }
  tmp <- tempfile(fileext = ".xlsx"); wb_save(out, tmp, overwrite = TRUE)
  push_xlsx_to_gsheet(tmp, gsheet_id)
}
...
build_module_summary <- function(data, all_cols, out_file, id_col, temporal_col,
  ...
  if (!is.null(gsheet_id) && gsheet_push_enabled()) push_xlsx_to_gsheet(out_file, gsheet_id)
  invisible(out_file)
}

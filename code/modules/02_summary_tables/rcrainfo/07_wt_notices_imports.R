...
# import-notice consent / waste stream), via the shared engine (00_engine.R).
...
# output/summary_tables/WIETS Imports Module Summary Tables.xlsx. The WIETS
# Exports + Imports workbooks share one Google Sheet, so the (optional)
# combined push at the bottom expects 06_wt_notices_exports.R to have run.
...
# WIETS Exports + Imports share one Google Sheet. Combine both formatted
# workbooks (Exports / Imports tab prefixes) and convert-upload as one Sheet.
# Run 06_wt_notices_exports.R first so its .xlsx exists on disk. Skipped
# unless RCRA_PUSH_GSHEET=true (see 00_engine.R).
if (gsheet_push_enabled()) {
  push_combined_to_gsheet(
    specs = list(
      list(path = "output/summary_tables/WIETS Exports Module Summary Tables.xlsx", prefix = "Exports "),
      list(path = "output/summary_tables/WIETS Imports Module Summary Tables.xlsx", prefix = "Imports ")
    ),
    gsheet_id = "10K6gJCvBhN5yBFIVpbJZSypC52ZdN3qeESxxyIFsKqc"
  )
}

...
- **Decision**
    - Panel membership is defined from the Biennial Report's own recognition of the facility, not from the facility's notified status in RCRAInfo. A facility-year is retained only if it satisfies `BR_GENERATOR == "L"` or `BR_TSDF == "Y"`.
- **Details**
    - LQG. A facility-year is an LQG when any of its waste lines in `BR_REPORTING_[year].csv` carries `CALCULATED_GENERATOR_STATUS == "L"`. This is stored in the panel as `BR_GENERATOR`, which is `"L"` when the condition holds and `"N"` otherwise. `CALCULATED_GENERATOR_STATUS` takes only three values in the raw files: `L` for a large quantity generator on the reported quantities, `N` for quantities below the LQG level, and `E` for an episodic-event filing.
    - TSDF. A facility-year is a TSDF when any of its waste lines carries `MGMT_ID_INCLUDED_IN_NBR == "Y"` or `RECV_ID_INCLUDED_IN_NBR == "Y"`, meaning EPA counted the facility as a manager or receiver of hazardous waste in the published Biennial Report totals for that cycle. This is stored as `BR_TSDF`.
    ...
    - It is therefore still possible for a facility-year to be a panel LQG while its notification history says otherwise. 4,966 facility-years (8.57% of the panel) have `BR_GENERATOR == "L"` together with an `HD_GENERATOR` that is not `"L"`, splitting into 2,492 notified as SQG (4.30%), 1,076 as VSQG (1.86%), 934 as non-generator (1.61%), 3 as undetermined (0.01%), and 461 with no notified category at all (0.80%).
    - It is likewise still possible for a facility-year to be a panel TSDF without notified TSD activity. 3,390 facility-years have `BR_TSDF == "Y"` together with `HD_TSDF == "N"`, which is 5.85 percent of the panel and 57.59 percent of the 5,886 TSDF facility-years. Recognition in the Biennial Report totals reflects waste that was actually managed or received that cycle, so it routinely reaches facilities whose standing notification does not (or no longer does) claim TSD activity.
...
- **Decision**
    - Every facility-year carries `FRS_ID`, the EPA Facility Registry Service `REGISTRY_ID`, so the panel can be joined to other EPA programs through a common facility identifier.
- **Details**
...
- **Details**
    ...
    - Assignment rule. Every source record sets an attribute's value from its `RECEIVE_DATE` forward, forming a step function per facility, and the value in force before January 1 carries into the year. A facility that switches on December 31 therefore keeps its January-through-December value for that year. Records sharing a `RECEIVE_DATE` are first collapsed to one value per the class rules below. The year's value is then assigned by class.
        - Ranked statuses (`HD_GENERATOR`, `HD_TSDF`, `HD_STATE_GENERATOR`) keep the value holding the most days of the calendar year; day ties break by severity (L over S over VS over N over P over U for the federal category, Y over N for TSD activity, and the federal hierarchy of Decision 8 for the state category).
        - Y/N activity indicators (everything from `HD_SHORT_TERM_GENERATOR` through the used-oil block) are severity-dominant. A facility classified as, for example, a spent lead-acid battery importer at any point of the calendar year is one for that facility-year; Y beats N regardless of days held.
...
- **Impact**
    ...
    - The coordinate fields are the exception and carry a real caveat. `HD_LOCATION_LATITUDE` is empty on 34,970 facility-years (60.34%) and `HD_LOCATION_LONGITUDE` on 34,977 (60.35%), because most notification records simply do not populate coordinates. Users needing geography should prefer state and county, or geocode through `FRS_ID`.
...
- NAICS codes (`NAICS4`, `NAICS6_1`-`NAICS6_4`)
    ...
    - **Details**
        ...
        - Multiple codes are treated as a feature of the notification form rather than a disagreement, so the NAICS columns sit outside `HD_CONFLICTS`; competing codes are ordered and selected by the priority rule above, which is itself the resolution. Every conflicted field that remains in the audit resolves under its own class rule (Decision 8).
    - **Considerations**
...
- **Decision**
    - Same-year disagreements in the facility's records are resolved by fixed class rules so that every `HD_*` attribute is single-valued, and every disagreement is preserved in the audit column `HD_CONFLICTS` rather than silently discarded.
- **Details**
    - A conflict means one `HD_*` field carries two or more distinct non-missing values among the records classified to the facility-year (classification per Decision 5: Biennial-Report-fed source types by `REPORT_CYCLE`, all others by receive year). `HD_CONFLICTS` lists the panel names of every such field, joined with semicolons in the panel's column order, and is empty when nothing conflicts. The NAICS columns sit outside the audit entirely, because multiple industry codes on one facility are a feature of the notification form rather than a disagreement (Decision 7).
    - Because the facility's Biennial Report filing classifies to its cycle year, a conflict typically means the values filed with the Biennial Report and the values notified during the same year disagree. This is the audit working as intended: the panel compares what the facility reported for the year against what it notified in the year.
    - A listed field means the inputs disagreed, not that the resolved value is in doubt. Resolution follows the assignment classes of Decision 5. Records sharing a `RECEIVE_DATE` first collapse to one value, most severe for the ranked statuses and higher status (Y over N) for the indicators; across the year, ranked statuses resolve by duration with severity ties, Y/N indicators resolve by severity outright, and plain attributes resolve by duration with recency ties.
    ...
    - One field receives an extra step. Any facility-year still flagged on `HD_GENERATOR` is overridden with `BR_GENERATOR`, because the Biennial Report is treated as authoritative for the federal generator category in a reporting year (Decision 2 established it as the membership basis).
- **Considerations**
    - The alternative of dropping conflicted facility-years was rejected because conflicts concentrate in exactly the active, frequently refiling facilities the panel cares about, and after the cycle-based classification they concentrate further in facility-years where the Biennial Report and the notifications disagree, which is information worth keeping. The audit column keeps the cost of resolution visible, so a user can exclude or re-resolve flagged facility-years if their analysis demands it.
- **Impact**
    - 8,229 of the 57,953 panel facility-years (14.20%) carry at least one conflict, covering 6,151 of the 23,014 facilities (26.73%).
    - The most conflicted fields, with the 57,953 panel facility-years as baseline, are `HD_STATE_GENERATOR` on 2,954 facility-years (5.10%), `HD_GENERATOR` on 2,518 (4.34%), `HD_LOCATION_LONGITUDE` on 1,641 (2.83%), `HD_LOCATION_LATITUDE` on 1,640 (2.83%), `HD_UNIVERSAL_WASTE_LQ_HANDLER` on 1,356 (2.34%), `HD_SHORT_TERM_GENERATOR` on 807 (1.39%), `HD_OFF_SITE_RECEIPT` on 609 (1.05%), and `HD_TSDF` on 524 (0.90%); every other field is below 0.70 percent.
    - The Biennial Report override applies to all 2,518 facility-years flagged on `HD_GENERATOR` (4.34% of the panel); on those facility-years `HD_GENERATOR` equals `BR_GENERATOR` by construction.

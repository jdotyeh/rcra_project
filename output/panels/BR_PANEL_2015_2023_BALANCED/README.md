...
- **Decision**
    - Panel membership is defined from the Biennial Report's own recognition of the facility, not from the facility's notified status in RCRAInfo. A facility-year qualifies when it satisfies `BR_GENERATOR == "L"` or `BR_TSDF == "Y"`, and a facility enters the panel only if some qualification holds in every one of the five cycles.
- **Details**
    ...
    - It is therefore still possible for a facility-year to be a panel LQG while its notification history says otherwise. 205 facility-years (0.75% of the panel) have `BR_GENERATOR == "L"` together with an `HD_GENERATOR` that is not `"L"`, splitting into 141 notified as SQG (0.52%), 28 as VSQG (0.10%), 28 as non-generator (0.10%), and 8 with no notified category at all (0.03%). The mismatch is an order of magnitude rarer than in the unbalanced panel (0.75% against 8.57%), because continuous filers keep their notifications current.
    - It is likewise still possible for a facility-year to be a panel TSDF without notified TSD activity. 1,969 facility-years have `BR_TSDF == "Y"` together with `HD_TSDF == "N"`, which is 7.24 percent of the panel and 47.85 percent of the 4,115 TSDF facility-years. Recognition in the Biennial Report totals reflects waste that was actually managed or received that cycle, so it routinely reaches facilities whose standing notification does not (or no longer does) claim TSD activity.
...
- **Impact**
    - 3,384 of the 27,200 panel facility-years (12.44%) carry at least one conflict, covering 1,906 of the 5,440 facilities (35.04%).

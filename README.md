...
3. **Panels** (`03_panels`): facility panels built from the master files and the Biennial Report — a balanced and an unbalanced facility-cycle panel of LQG/TSDF facilities (2015–2023), a facility-month compliance-evaluation panel (2015–2023), a facility-month enforcement-action panel (2015–2023), and a facility-month determined-violation panel (2015–2023) — each linked to EPA FRS IDs.
...
| `output/panels/CE_PANEL_2015_2023/*` | derived | Facility-month evaluation, enforcement, and violation panels | No (large, rebuilt by code) |
...
- Programs in `code/modules/03_panels/rcrainfo/` build the facility panels under `output/panels/` (all link `FRS_ID` from the FRS Program Links file):
  ...
  - `03_panel_eval_2015_2023.R` — balanced facility-month panel (`EVAL_PANEL_2015_2023.csv`) of RCRA compliance evaluations from `CE_MASTER`, placed on the month an evaluation started, covering every month from January 2015 through December 2023. It carries per-month evaluation counts by type with their 0/1 indicators, a count of the month's evaluations that found violations, the conducting agency with its state and its state-prefixed suborganization codes (for example `IL-CD`), the distinct evaluation end dates with their count, and citizen-complaint and multimedia-inspection indicators.
  - `04_panel_enf_2015_2023.R` — balanced facility-month panel (`ENF_PANEL_2015_2023.csv`) of RCRA enforcement actions from `CE_MASTER`, placed on the month an action was issued, covering the same months. It carries per-month action counts split by issuing agency (state or EPA), the nationally-defined type codes each action recodes to beside the state-specific codes as recorded, the matching type descriptions and proposed categories, formal and informal indicators, and the docket numbers and state-prefixed suborganization codes.
  - `05_panel_viol_2015_2023.R` — balanced facility-month panel (`VIOL_PANEL_2015_2023.csv`) of determined RCRA violations from `CE_MASTER`, placed on the month a violation was determined, covering the same months. It carries per-month violation counts split by determining agency (state or EPA) and by violation type, counts of the violations still open, the owner-prefixed regulatory citations (for example `HQ-262.34(a)`) with their origin codes and count, the return-to-compliance dates, the agency responsible afterwards, and counts of the evaluations and enforcement actions linked to the month's violations.
  - `build_panels.R` — shortcut that builds only the panels end to end, running setup, the FRS download and the RCRAInfo download if the raw inputs are missing, the `HD_MASTER` and `CE_MASTER` master files if they are missing, and then the five panels. Excluded from `master.R` so a full run does not build the panels twice.
